...
from models.full_models import UTranscalib_mobilenet_simple
from config.model_config import *

from quaternion_distances import quaternion_distance

# from tensorboard X import Summary Writer
...
ex = Experiment("UTransCalib_mobilenet")
ex.captured_out_filter = apply_backspaces_and_linefeeds

MODEL_CONFIG = DotWiz(config_UTransCalib_mobilenet_oct24)
MODEL_DATE = datetime.now().strftime('%Y%m%d_%H%M%S')
MODEL_NAME = MODEL_CONFIG.model_name # + '_alternative2'
...
def config():
    ...
    data_folder = '/home/indowicom/umam/ext_auto_calib_camlid/ETRI_Project_Auto_Calib/datasets/KITTI-Odometry/dataset'
    use_reflectance = False
    val_sequence = 0
    epochs = 200
    BASE_LEARNING_RATE = 1e-6  # 1e-4
    loss = 'combined'
    ...
    optimizer = 'adam'
    resume = True
    weights = './checkpoints/kitti/odom/val_seq_00/models/checkpoint_UTransCalib_mobilenet_oct24_r1.00_t0.10_e151_0.048.tar'
    #'./fixed_weights/main_model/checkpoint_UTransCalib_mobilenet_oct24_r5.00_t0.50_e111_0.112.tar'
    rescale_rot = 1.0
    rescale_transl = 2.0
    precision = "O0"
    norm = 'bn'
...
comet_log.set_name(f'{MODEL_NAME}_{MODEL_DATE}')
...
def lidar_project_depth(pc_rotated, cam_calib, img_shape):
    ...
    depth_img = depth_img.to(device=device)
...
def main(_config, _run, seed):
    ...
    if _config['network'].startswith('Res'):
        ...
        model = UTranscalib_mobilenet_simple(MODEL_CONFIG)
    else:
    ...
    for epoch in range(starting_epoch, _config['epochs'] + 1):
        ...
        train_process = tqdm(TrainImgLoader, unit='batch')

        for batch_idx, sample in enumerate(train_process):
            ...
            # gt pose
            sample['tr_error'] = sample['tr_error'].to(device=device)
            sample['rot_error'] = sample['rot_error'].to(device=device)
            ...
            for idx in range(len(sample['rgb'])):
                ...
                sample['point_cloud'][idx] = sample['point_cloud'][idx].to(device=device) # 变换到相机坐标系下的激光雷达点云
                ...
                rgb = sample['rgb'][idx].to(device=device)
                shape_pad = [0, 0, 0, 0]
            ...
            lidar_input = F.interpolate(lidar_input, size=[192, 640], mode="bilinear")
            ...
            # log to comet   
            comet_log.log_metrics({'batch_total_loss': loss['total_loss'].item(),
                                   'batch_translation_loss': loss['transl_loss'].item(),
                                   'batch_rotation_loss': loss['rot_loss'].item()})
            if _config['loss'] == 'combined':
                comet_log.log_metric("batch_pcd_loss", loss['point_clouds_loss'].item())
            # ###
            ...
            train_process.set_postfix(loss=loss['total_loss'].item())
        ...
        val_process = tqdm(ValImgLoader, unit='batch')

        for batch_idx, sample in enumerate(val_process):
            #print(f'batch {batch_idx+1}/{len(TrainImgLoader)}', end='\r')
            start_time = time.time()
            lidar_input = []
            rgb_input = []
            ...
            for idx in range(len(sample['rgb'])):
                ...
                if _config['use_reflectance']:
                    reflectance = sample['reflectance'][idx].to(device=device)
            ...
            val_process.set_description('Validation: ')
            val_process.set_postfix(loss=loss['total_loss'].item())
        ...
        # log to comet   
        comet_log.log_metrics({'val_total_loss': total_val_loss / len(dataset_val),
                                'val_translation_error': total_val_t / len(dataset_val),
                                'val_rotation_error': total_val_r / len(dataset_val)})
        # ###
        ...
        if val_loss < BEST_VAL_LOSS:
            ...
            savefilename = f'{model_savepath}/checkpoint_{MODEL_NAME}_r{_config["max_r"]:.2f}_t{_config["max_t"]:.2f}_e{epoch}_{val_loss:.3f}.tar'
            torch.save({
                'config': _config,
                'epoch': epoch,
