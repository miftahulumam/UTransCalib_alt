...
# from models.MRCNet import MRCNet
...
ex = Experiment("MRCNet-evaluate-iterative")
...
def config():
    ...
    output = './output_finest_main_des9'
    save_image = False
    outlier_filter = True
...
def main(_config, seed):
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        for idx in range(len(sample['rgb'])):
            ...
            sample['point_cloud'][idx] = sample['point_cloud'][idx].to(device)  # 变换到相机坐标系下的激光雷达点云
        ...
        with torch.no_grad():
            for iteration in range(start, len(weights)):
                ...
                lidar_resize = F.interpolate(lidar, size=[192, 640], mode="bilinear")
